...
namespace SMP.DAL.Repos.Base
{
    public abstract class RepoBase<T> : IDisposable, IRepo<T> where T : EntityBase, new()
    {
        ...
        protected RepoBase(DbContextOptions<Context> options)
        {
        ...
        {
            entity.TimeStamp = BitConverter.GetBytes(DateTime.Now.ToBinary());
            Table.Add(entity);
        ...
        public virtual int Update(T entity, bool persist = true)
        {
            ...
            entity.TimeStamp = Table.AsNoTracking()
                .Where(w => w.Id == entity.Id)
                .FirstOrDefault().TimeStamp;
            //Console.WriteLine(" >>---------> " + entity.Id + " / " + entity.TimeStamp);
            Table.Update(entity);
            return persist ? SaveChanges() : 0;
        }
